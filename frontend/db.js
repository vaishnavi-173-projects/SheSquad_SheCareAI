...
// ── uploadPDF ──────────────────────────────────────────────
/**
 * Upload a PDF Blob to the 'reports' Supabase Storage Bucket.
 * @param {Blob} fileBlob - The PDF file blob generated by the backend
 * @param {string} userId - ID of the authenticated user
 * @returns {string|null} - Public URL of the uploaded PDF or null if failed
 */
async function uploadPDF(fileBlob, userId) {
  const sb = getSupabase();
  if (!sb) return null;

  const fileName = `${userId}/report_${Date.now()}.pdf`;
  ...
  try {
    ...
    console.log('[Storage] ✅ PDF Upload success. URL:', urlData.publicUrl);
    return urlData.publicUrl;
  } catch (e) {
    console.error('[Storage] ❌ uploadPDF error:', e.message);
    return null;
  }
}
